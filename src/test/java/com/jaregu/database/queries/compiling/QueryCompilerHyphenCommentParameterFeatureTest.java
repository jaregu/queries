...
public class QueryCompilerHyphenCommentParameterFeatureTest {

	private QueryCompilerHyphenCommentParameterFeature feature = new QueryCompilerHyphenCommentParameterFeature();
	...
	public void testIsCompilable() throws Exception {
		...
		context.withContext(() -> {
			...
			assertEquals(true, feature.isCompilable(() -> asList(create("aaa"), create("-- some comment\n"))));
	...
	private void testOneExpressionTest(boolean fullValues) {
		...
		when(expression.getVariableNames()).thenReturn(Arrays.asList("one", "two"));
		when(expression.eval(resolver)).thenReturn("VALUE");
		when(resolver.getValue("one")).thenReturn("full1");
		when(resolver.getValue("two")).thenReturn(fullValues ? "full2" : null);
		...
		Result result = feature.compile(() -> asList(create("before (replace me) "), create("-- expr  \n")), compiler);
		...
		if (fullValues) {
			...
			assertEquals("before ? " + getArgumentCommentIfEnabled("(replace me)") + "-- expr  \n", sqlHoler.get());
	...
	private void testConditionalExpressionTest(boolean condition) {
		...
		when(expressionParser.parse("expr ")).thenReturn(valueExpression);
		...
		AtomicReference<String> sqlHoler = new AtomicReference<>();
		AtomicReference<List<Object>> parametersHolder = new AtomicReference<>();
		Result result = feature.compile(() -> asList(create("before (replace me) "), create("-- expr ; condition \n")),
				compiler);
		CompiledQueryPart queryPart = result.getCompiledParts().get(0);
		queryPart.eval(resolver, (sql, parameters) -> {
			executed.set(true);
		...
		if (condition) {
			assertTrue(executed.get());
			assertEquals("before ? " + getArgumentCommentIfEnabled("(replace me)") + "-- expr ; condition \n",
					sqlHoler.get());
			assertEquals(1, parametersHolder.get().size());
			assertEquals("VALUE", parametersHolder.get().get(0));
		} else {
